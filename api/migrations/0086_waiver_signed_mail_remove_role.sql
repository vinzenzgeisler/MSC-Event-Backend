-- The "Rolle: {{signerRole}}" line printed the raw internal role value (e.g. "driver" /
-- "codriver") to the recipient, which is meaningless to a driver and reads like a stray
-- debug tag to a co-driver receiving the same mail. Drop the line; the mail is already
-- addressed personally by name and the role adds no useful information for the recipient.
...
where not exists (
  ...
  where v.template_id = tt.template_id
    ...
    and v.body_text_template not like '%Rolle:%'
);
